...
0.3
0.4
0.5Overall Performance (Edit Distance)
InternVL2-76B
Qwen2.5-VL-7B
OLMOCR
OCRFlux-3B
...
Qwen2(.5)/3VL series...
[× ] too many vision tokens
[× ] large activations[× ] small global view
...
knowledge with dense global attention, and a 16×token compressor that bridges between them.
3. Methodology
3.1. Architecture
...
from Vary [36] and use a 2-layer convolutional module to perform 16×downsampling of vision
tokens. Each convolutional layer has a kernel size of 3, stride of 2, padding of 1, and channels
increase from 256 to 1024. Assuming we input a 1024×1024 image, the DeepEncoder will seg-
ment it into 1024/16×1024/16=4096 patch tokens. Since the first half of encoder is dominated by
...
resolutions and token counts of 512×512 (64), 640×640 (100), 1024×1024 (256), and 1280×1280
...
𝑁𝑣𝑎𝑙𝑖𝑑 =⌈𝑁 𝑎𝑐𝑡𝑢𝑎𝑙 ×[1−((𝑚𝑎𝑥(𝑤,ℎ)−𝑚𝑖𝑛(𝑤,ℎ))/(𝑚𝑎𝑥(𝑤,ℎ)))]⌉(1)
where𝑤andℎrepresent the width and height of the original input image.
6
...
𝑛is the number of tiles. For images with both width and height smaller than 640, 𝑛is set to 0,
...
views+1280×1280 global view) is obtained through continued training on a trained DeepSeek-
...
𝑓dec :R 𝑛×𝑑latent →R 𝑁×𝑑text ; ˆX=𝑓 dec (Z)where𝑛≤𝑁(2)
whereZ ∈R 𝑛×𝑑latent are the compressed latent(vision) tokens from DeepEncoder and ˆX∈R 𝑁×𝑑text
...
directly from the full dataset usingfitz, aimed at teaching the model to recognize optical text,
...
input images to 512×512 and 640×640.
Text Tokens
...
"†200dpi" means usingfitzto interpolate the original image to 200dpi. For the DeepSeek-OCR
...
=== Page 12 ===
a feature of the forgetting mechanism. When compressing tokens by nearly 20×, we find that
...
beyond𝑘rounds in multi-turn conversations to achieve 10×compression efficiency.
18
...
[20] A. Liu, B. Feng, B. Xue, B. Wang, B. Wu, C. Lu, C. Zhao, C. Deng, C. Zhang, C. Ruan, et al.
...
[22] I. Loshchilov and F. Hutter. Sgdr: Stochastic gradient descent with warm restarts. arXiv
preprint arXiv:1608.03983, 2016.
[23] I. Loshchilov and F. Hutter. Decoupled weight decay regularization. In ICLR, 2019.
[24] A. Masry, D. X. Long, J. Q. Tan, S. Joty, and E. Hoque. Chartqa: A benchmark for question
...
=== Page 22 ===
[32] A. Singh, V . Natarajan, M. Shah, Y. Jiang, X. Chen, D. Batra, D. Parikh, and M. Rohrbach. To-
...
[33] T. Sun, C. Cui, Y. Du, and Y. Liu. Pp-doclayout: A unified document layout detection model
to accelerate large-scale data construction. arXiv preprint arXiv:2503.17213, 2025.
[34] B. Wang, C. Xu, X. Zhao, L. Ouyang, F. Wu, Z. Zhao, R. Xu, K. Liu, Y. Qu, F. Shang, et al.
...
[39] H. Wei, Y. Yin, Y. Li, J. Wang, L. Zhao, J. Sun, Z. Ge, X. Zhang, and D. Jiang. Slow perception:
...
[41] W. Yu, Z. Yang, L. Li, J. Wang, K. Lin, Z. Liu, X. Wang, and L. Wang. Mm-vet: Evaluating
